...
stamp="${1:?usage: wait_launch_prompt_mean_formal.sh YYYYMMDD_HHMMSS}"
resume_log="/nfs-medical3/zyh/v4/phase6/logs/prompt_mean_prototype_resume_20260727_070931.log"
resume_checkpoint="/nfs-medical3/zyh/v4/phase6/formal_full_runs/phase6_joint_pixel_20260727_070931/checkpoint_epoch_001.pth"
gpu_ids="1,3,4,5"

until [[ -f "$resume_checkpoint" ]] && grep -q '^EXIT_CODE=0$' "$resume_log"; do
  printf '%s waiting_for_resume_gate checkpoint=%s\n' "$(date '+%F %T')" "$resume_checkpoint"
  sleep 60
done
