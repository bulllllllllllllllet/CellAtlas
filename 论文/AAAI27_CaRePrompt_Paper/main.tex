...
\end{equation}
This projection also couples pixel supervision to region geometry during joint fine-tuning. When the differentiable region-assignment pathway is trainable, the pixel loss differentiates through $\hat{M}$ with respect to $A_f(x,k)$ as well as $p_k$. Thus, if a boundary pixel is consistently assigned to a region with an incompatible target probability, its error can increase the membership of a better-matched region and decrease the competing membership, shifting the soft region boundary toward the tissue contour rather than only changing region scores. Prompt--region associations and region targets are recomputed from the current assignment at every iteration, while balance, entropy, compactness, and prompt-geometry constraints regularize this update and prevent degenerate boundary drift. When this pathway is frozen, the decoder instead refines only $p_k$ over fixed regions.
