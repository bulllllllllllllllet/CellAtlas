...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{images/main_picture_compressed.pdf}
...
We split 200 H\&E WSIs into 140/30/30 training/validation/test slides; all comparisons use the sealed test set, while model selection, tuning, and ablations use validation only. The fixed ablation manifest has 4,000 episodes, while each training epoch contains 20,000 episodes. An episode specifies a WSI region, a target tissue, its binary mask, positive prompts, and optional hard negative prompts. Unless stated otherwise, all variants share the split, prompt manifest, random seed, augmentation, and optimization schedule. Components downstream of an ablated representation are retrained to avoid distribution mismatch with the full-model decoder.
