...
\subsection{Cell Representation Learning}
...
\subsection{Multiscale Learnable Tissue Regions}
...
\subsection{Cell-Aware Region Representations}
...
\subsection{Context-Aware Decoding and Mask Recovery}
...
\subsection{Staged Episodic Training}
...
\subsection{Comparison with Promptable Segmentation Models}
...
FewClick obtains 0.7495 macro Dice, surpassing WSI-SAM, the strongest baseline on this metric, by 0.3153. It also improves macro mIoU and micro Dice by 0.3138 and 0.1819, respectively. SAM attains high recall but low precision, indicating extensive false positives, whereas FewClick maintains balanced precision and recall. FewClick ranks first for all 12 tissues, with particularly large gains on normal gland, normal stroma, and lymphocyte aggregates, where cellular composition and contextual organization are informative. Necrosis and blood remain the most challenging classes.

\subsection{Ablation Studies}
We isolate four completed architectural ablations under the fixed 4,000-episode validation protocol. All results in this section are used only for validation-time model analysis; the sealed test split is not read. Removing or simplifying any component reduces performance, confirming that the gains do not arise from a single branch.
...
Replacing the learned, signed Set Encoder with masked positive and negative mean prototypes reduces macro Dice from 0.7413 to 0.7215 and micro Dice from 0.8138 to 0.7970; Boundary F1 also decreases from 0.3142 to 0.3062. This variant uses the same prompt-tail and decoder training protocol, so the difference isolates the aggregation rule rather than downstream adaptation. The result indicates that learned permutation-invariant aggregation retains prompt-set structure beyond a first-order prototype, particularly when positive and negative evidence must be combined to define a tissue concept. Its 0.05 prompt-conflict rate is below the fixed-region control, but no checkpoint satisfies the validation-only selection criteria; we therefore treat this as evidence for the Prompt Set Encoder rather than a deployable alternative.
