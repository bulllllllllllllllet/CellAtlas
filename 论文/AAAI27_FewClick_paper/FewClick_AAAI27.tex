...
FewClick requires aligned 12-tissue masks and cell annotations unavailable together in the public prompt-segmentation benchmarks, so we use a private colorectal H\&E cohort whose 200 WSIs are split by patient into 140/30/30 train/validation/test cases; the test set is unused, and the independent TLS case has six polygons (three prompted and three retrieval-only).

\paragraph{Hyperparameter selection.}
During Phase-6 development, we tried backbone learning rates $\{2.5{\times}10^{-7},10^{-6}\}$, decoder/context learning rates $\{2.5{\times}10^{-5},10^{-4}\}$, and three- or five-epoch budgets, selecting the final checkpoint on validation macro/micro Dice subject to fixed gates of $0.72/0.7987$ followed by Dice-priority Pareto selection.

\paragraph{Randomness and runs.}
Each configuration uses one deterministic seeded training run; patch evaluation uses one frozen manifest, while the whole-slide result averages five prompt sets frozen before inference. Seed 20260720 initializes Python, NumPy, PyTorch, and all CUDA devices, with the distributed rank added to the training seed.

\paragraph{Computing environment.}
Experiments used Ubuntu 22.04.4, two AMD EPYC 7Y83 CPUs (128 cores), 503\,GiB RAM, and up to four NVIDIA RTX 5880 Ada GPUs (48\,GiB; driver 550.67), with Python 3.10.19, PyTorch 2.9.1, CUDA 12.8, NumPy 2.2.6, pandas 2.3.3, and SciPy 1.15.3.

\paragraph{Metrics and statistics.}
We use $\mathrm{Dice}=2TP/(2TP+FP+FN)$ and $\mathrm{IoU}=TP/(TP+FP+FN)$ to measure region overlap, compute macro scores by averaging the 12 tissue-class scores and micro scores from pooled pixels, and complement overlap with Boundary F1 obtained by matching predicted and reference boundary pixels within two pixels.
On the 885 paired common episodes under $(1{+}1)$ prompts, FewClick's episode-Dice gain over the strongest external baseline SAM has median $0.2485$ (IQR $0.0545$--$0.5253$), and a two-sided paired Wilcoxon signed-rank test gives $W=36{,}179$ and $p=4.84{\times}10^{-98}$; class-wise WSI scores are also reported to expose performance variation, while ablation differences remain descriptive.

\paragraph{Model and optimization.}
FewClick uses 64 regions, 256-D tokens, at most 255 cells, and a two-layer, four-head, six-neighbor graph decoder (dropout 0.1); Phase-2 region learning, Phase-3 cell fusion, Phase-5 prompt learning, and initial joint fitting each run for 30 epochs, followed by three-epoch prompt/decoder stabilization and five-epoch J5 refinement that updates only backbone layer~4 at LR $10^{-6}$ with batch size 2 per GPU, with all stage-specific settings provided in the released configurations.
\paragraph{Baseline inference.}
Baselines use released weights without adaptation and threshold 0.5; SAM and SAM-Med2D select the highest-predicted-IoU mask (SAM-Med2D input size 256), whereas WSI-SAM returns one fused mask.

\paragraph{Code availability.}
All code and frozen configurations will be released under the MIT License upon publication.
\end{document}
